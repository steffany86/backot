...
public class SupervisionRepository {
    ...
    public List<Map<String, Object>> listarTecnicosPorSupervisor(Integer idSupervisor, String sucursal) {
        ...
        try {
            ids.addAll(central.queryForList(SP_LISTAR_TECNICOS_SUPERVISOR_CENTRAL, idSupervisor, trimToNull(sucursal)));
        } catch (Exception ignored) {
            // fallback below with tabla directa
        }
        ids.addAll(cargarTecnicosDesdeConformacionDiaria(central, idSupervisor, sucursal));
    ...
    private List<Map<String, Object>> cargarTecnicosDesdeConformacionDiaria(
        ...
        String[] sqlCandidates = new String[] {
                "SELECT DISTINCT id_tecnico AS idTecnico " +
                        "FROM dbo.tbl_ConformacionCuadrillaDiario " +
                        "WHERE ISNULL(e_eliminado,0)=0 " +
                        "  AND CONVERT(date, fecha)=CONVERT(date, GETDATE()) " +
                        "  AND (id_supervisor=? OR id_encargado=? OR supervisor=?) " +
                        "  AND (? IS NULL OR LTRIM(RTRIM(ISNULL(sucursal,''))) = LTRIM(RTRIM(?)))",
                "SELECT DISTINCT idTecnico AS idTecnico " +
                        "FROM dbo.tbl_ConformacionCuadrillaDiario " +
                        "WHERE ISNULL(e_eliminado,0)=0 " +
                        "  AND CONVERT(date, fecha)=CONVERT(date, GETDATE()) " +
                        "  AND (idSupervisor=? OR idEncargado=? OR supervisor=?) " +
                        "  AND (? IS NULL OR LTRIM(RTRIM(ISNULL(sucursal,''))) = LTRIM(RTRIM(?)))",
                "SELECT DISTINCT id_tecnico AS idTecnico " +
                        "FROM dbo.tbl_ConformacionCuadrillaDiario " +
