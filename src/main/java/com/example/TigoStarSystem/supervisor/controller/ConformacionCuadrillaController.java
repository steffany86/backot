...
import com.example.TigoStarSystem.supervisor.dto.ConformacionCuadrillaEstadoRequest;
...
public class ConformacionCuadrillaController {
    ...
    /**
     * Actualiza solo el estado (ACTIVO/AUSENTE) de una cuadrilla ya confirmada,
     * sin modificar el resto de columnas de tbl_ConformacionCuadrillaDiario.
     */
    @PatchMapping("/{id}/estado")
    public ResponseEntity<ApiResponse<Integer>> actualizarEstado(
            @PathVariable("id") Long id,
            @RequestBody ConformacionCuadrillaEstadoRequest request) {
        int filas = service.actualizarEstado(id, request.getEstado(), request.getEliminado());
        return ResponseEntity.ok(ApiResponse.of(
                filas,
                "Estado de la cuadrilla actualizado."
        ));
    }
