...
public class ConformacionCuadrillaService {
    ...
    /**
     * Actualiza unicamente estado (ACTIVO/AUSENTE) y e_eliminado de una cuadrilla ya
     * confirmada. No toca ninguna otra columna de tbl_ConformacionCuadrillaDiario.
     * La creacion de registros sigue insertando e_eliminado=0 sin cambios.
     */
    public int actualizarEstado(Long id, String estadoRaw, Boolean eliminadoRaw) {
        validarId(id);
        String estado = estadoRaw == null ? "" : estadoRaw.trim().toUpperCase(Locale.ROOT);
        if (!"ACTIVO".equals(estado) && !"AUSENTE".equals(estado)) {
            throw new ApiException(
                    HttpStatus.BAD_REQUEST,
                    "VALIDATION_ERROR",
                    "estado invalido: solo se permite ACTIVO o AUSENTE."
            );
        }
        boolean eliminado = Boolean.TRUE.equals(eliminadoRaw);
        int affected = repository.actualizarEstado(id, estado, eliminado);
        if (affected <= 0) {
            throw new ApiException(
                    HttpStatus.NOT_FOUND,
                    "REGISTRO_NO_ENCONTRADO",
                    "No se encontro la cuadrilla confirmada para actualizar el estado."
            );
        }
        return affected;
    }
    ...
    private Set<String> obtenerClavesConfirmadas(List<Map<String, Object>> confirmadas) {
        ...
        for (Map<String, Object> row : confirmadas) {
            // Una confirmacion eliminada no debe bloquear que la cuadrilla
            // vuelva a aparecer como pendiente para registrarla nuevamente.
            if (rowMapper.isEliminado(row)) {
                continue;
            }
    ...
    private void validarTecnicoNoDuplicadoEnBd(
        ...
        Map<String, Object> existente = repository.buscarRegistroActivoPorTecnicoEnCentral(
                fecha,
                sucursal,
                tecnicoValue,
                idExcluir
