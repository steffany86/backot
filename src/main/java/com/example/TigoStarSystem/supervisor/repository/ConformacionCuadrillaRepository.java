...
public class ConformacionCuadrillaRepository {
    ...
    public int guardarFilaConfirmada(ConformacionCuadrillaRowRequest fila) {
        ...
        String supervisorConfirmo = fila == null ? null : asTrimmedText(fila.getSupervisorConfirmo());
        if (isBlank(supervisorConfirmo)) {
            supervisorConfirmo = obtenerNombreUsuarioPorId(
        ...
        if (!guardadoLocal && !esDuplicadoConformacion(errorLocal)) {
            if (errorLocal != null) {
                throw errorLocal;
            }
    ...
    private boolean esDuplicadoConformacion(RuntimeException error) {
        if (error == null || error.getMessage() == null) {
            return false;
        }
        String mensaje = error.getMessage().toLowerCase(Locale.ROOT);
        return mensaje.contains("ya esta registrado")
                || mensaje.contains("already exists")
                || mensaje.contains("duplicate")
                || mensaje.contains("duplicado")
                || mensaje.contains("conformacioncuadrilladiario");
    }
    ...
    /**
     * Actualiza unicamente estado y e_eliminado de una cuadrilla ya confirmada,
     * sin tocar el resto de columnas de tbl_ConformacionCuadrillaDiario.
     */
    public int actualizarEstado(Long id, String estado, boolean eliminado) {
        if (centralJdbcTemplate == null) {
            throw new IllegalStateException("No hay datasource central configurado para actualizar confirmaciones.");
        }
        return centralJdbcTemplate.update(
                "UPDATE dbo.tbl_ConformacionCuadrillaDiario SET estado = ?, e_eliminado = ? WHERE id = ?",
                estado,
                eliminado,
                id
        );
    ...
    /** Busca duplicados únicamente en BDControlOrdenes. */
    public Map<String, Object> buscarRegistroActivoPorTecnicoEnCentral(
            LocalDate fecha,
            String sucursal,
            String tecnico,
            Long idExcluir) {
    ...
    private int ejecutarRegistrar(JdbcTemplate template, ConformacionCuadrillaRowRequest fila) {
        // Un id_tecnicoAuxiliar sin su nombre (auxiliar en blanco) es la firma de que en
        // realidad no se selecciono auxiliar: se fuerza a 0 para no persistir ese id suelto.
        boolean auxiliarSinNombre = isBlank(asTrimmedText(fila.getAuxiliar()));
        Integer idTecnicoAuxiliar = (fila.getIdTecnicoAuxiliar() == null || auxiliarSinNombre)
                ? Integer.valueOf(0)
