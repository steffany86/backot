...
public class OtController {
    ...
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> listarOtOrdenesPasadasMaterial(
            @RequestHeader(value = "X-Session-Token", required = false) String token,
            @RequestParam(value = "usuario", required = false) Integer idUsuario) {
        AuthMeResponse me = resolveSession(token);
        Integer idSucursal = extractIdSucursal(me);
        ...
        if (me != null && me.getUsuario() != null) {
            idUsuarioFiltro = me.getUsuario().getIdUsuario();
            rol = me.getUsuario().getRol();
            nombre = me.getUsuario().getNombre();
        }

        return ResponseEntity.ok(ApiResponse.of(
                otService.listarOrdenesPasadasMaterial(idUsuarioFiltro, idSucursal, rol, nombre),
                "Listado de OT pendientes de registro de material."
        ));
    }
    ...
    @GetMapping("/modificar-hoy")
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> listarOtModificablesHoy(
            @RequestHeader(value = "X-Session-Token", required = false) String token) {
        AuthMeResponse me = requireModificarOtRole(token);
        Integer idSucursal = extractIdSucursal(me);
        Integer idUsuario = me.getUsuario().getIdUsuario();
        return ResponseEntity.ok(ApiResponse.of(
                otService.listarFinalizadasModificablesHoy(idUsuario, idSucursal),
                "OT finalizadas modificables de hoy."
        ));
    }

    @GetMapping("/{id}/modificar-hoy")
    public ResponseEntity<ApiResponse<Map<String, Object>>> obtenerOtModificableHoy(
            @RequestHeader(value = "X-Session-Token", required = false) String token,
            @PathVariable("id") Long idVenta) {
        AuthMeResponse me = requireModificarOtRole(token);
        Integer idSucursal = extractIdSucursal(me);
        Integer idUsuario = me.getUsuario().getIdUsuario();
        return ResponseEntity.ok(ApiResponse.of(
                otService.obtenerFinalizadaModificableHoy(idVenta, idUsuario, idSucursal),
                "OT finalizada modificable de hoy."
        ));
    }

    @PutMapping("/{id}/cantidades-hoy")
    public ResponseEntity<ApiResponse<Map<String, Object>>> modificarCantidadesHoy(
            @RequestHeader(value = "X-Session-Token", required = false) String token,
            @PathVariable("id") Long idVenta,
            @RequestBody OtModificarCantidadesRequest request) {
        AuthMeResponse me = requireModificarOtRole(token);
        Integer idSucursal = extractIdSucursal(me);
        Integer idUsuario = me.getUsuario().getIdUsuario();
        return ResponseEntity.ok(ApiResponse.of(
                otService.modificarCantidadesHoy(idVenta, request, idUsuario, idSucursal),
                "Cantidades de materiales actualizadas correctamente."
        ));
    }

    @PostMapping("/{id}/modificar-hoy/materiales")
    public ResponseEntity<ApiResponse<Map<String, Object>>> agregarMaterialModificarHoy(
            @RequestHeader(value = "X-Session-Token", required = false) String token,
            @PathVariable("id") Long idVenta,
            @RequestBody OtDetalleMaterialRequest request) {
        AuthMeResponse me = requireModificarOtRole(token);
        Integer idSucursal = extractIdSucursal(me);
        Integer idUsuario = me.getUsuario().getIdUsuario();
        return ResponseEntity.ok(ApiResponse.of(
                otService.agregarMaterialModificarHoy(idVenta, request, idUsuario, idSucursal),
                "Material agregado correctamente."
        ));
    }

    private AuthMeResponse requireModificarOtRole(String token) {
        AuthMeResponse me = resolveSession(token);
        String rol = me == null || me.getUsuario() == null || me.getUsuario().getRol() == null
                ? "" : me.getUsuario().getRol().trim().toLowerCase().replace(" ", "").replace("_", "");
        if (!"tecnico".equals(rol) && !"sistemas".equals(rol) && !"backofficev".equals(rol)) {
            throw new ApiException(HttpStatus.FORBIDDEN, "FORBIDDEN_MODIFICAR_OT", "Solo técnico, sistemas y backoffice_v pueden modificar cantidades de OT.");
        }
        return me;
    }
