...
public class ListaOtRepository {
    ...
    public List<Map<String, Object>> listarPorFecha(LocalDate fecha, String tecnico, Integer idSucursal, Integer idUsuario) {
        ...
        JdbcTemplate templateOt = centralJdbcTemplate != null ? centralJdbcTemplate : templateSucursal;
        ...
        // idUsuario (sesion) -> idVendedor -> salesForce -> SP BO CITA MAKIRO
        // encapsulado en un SP dedicado.
        if (idUsuario != null && idUsuario > 0) {
            try {
                List<Map<String, Object>> rowsByUsuario = templateOt.queryForList(
                        "EXEC dbo.spx_ListarOtPorUsuarioFecha ?, ?",
                        idUsuario,
                        fechaLegacy
                );
                logger.info("{{\"evento\":\"LISTA_OT_SP_USUARIO_FECHA\",\"idUsuario\":{},\"fecha\":\"{}\",\"rows\":{}}}",
                        idUsuario, fechaLegacy, rowsByUsuario == null ? 0 : rowsByUsuario.size());
                mergeRows(mergedRows, rowsByUsuario);
                if (!mergedRows.isEmpty()) {
                    return new ArrayList<>(mergedRows.values());
                }
            } catch (DataAccessException ex) {
                logger.warn("{{\"evento\":\"LISTA_OT_SP_USUARIO_FECHA_FALLBACK\",\"idUsuario\":{},\"fecha\":\"{}\",\"error\":\"{}\"}}",
                        idUsuario, fechaLegacy, ex.getMessage());
        ...
        if (tecnicoParam != null || (idUsuario != null && idUsuario > 0)) {
            List<String> salesforceDesdeUsuario = obtenerSalesforcePorIdUsuario(idUsuario, idSucursal);
            logger.info("{{\"evento\":\"LISTA_OT_FLUJO_USUARIO_VENDEDOR\",\"idUsuario\":{},\"idSucursal\":{},\"tecnicoSesion\":\"{}\",\"salesforce\":{}}}",
                    idUsuario, idSucursal, tecnicoParam, salesforceDesdeUsuario);
    ...
    public List<String> obtenerSalesforcePorIdUsuario(Integer idUsuario, Integer idSucursal) {
        ...
        String sql =
                "SELECT DISTINCT " +
                "LTRIM(RTRIM(ISNULL(v.salesForce, ''))) AS salesForce " +
                "FROM dbo.tbl_UsuarioTecnico ut " +
        ...
        try {
            List<Map<String, Object>> rows = target.queryForList(sql, idUsuario);
            if (rows == null || rows.isEmpty()) {
                logger.info("{{\"evento\":\"LISTA_OT_SALESFORCE_VACIO\",\"idUsuario\":{},\"idSucursal\":{}}}", idUsuario, idSucursal);
                return Collections.emptyList();
            }
            ...
            logger.info("{{\"evento\":\"LISTA_OT_SALESFORCE_OK\",\"idUsuario\":{},\"idSucursal\":{},\"salesforce\":{}}}",
                    idUsuario, idSucursal, salesforceList);
            return new ArrayList<>(salesforceList);
        } catch (DataAccessException ex) {
            logger.warn("{{\"evento\":\"LISTA_OT_SALESFORCE_ERROR\",\"idUsuario\":{},\"idSucursal\":{},\"error\":\"{}\"}}",
                    idUsuario, idSucursal, ex.getMessage());
            return Collections.emptyList();
        }
    }
