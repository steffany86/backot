...
public class CuadreAutomaticoMailService {
    ...
    private String crearCuerpo(Map<String, Object> resultado, LocalDate fecha) {
        ...
        List<Map<String, Object>> validacionesSaldo = resultado == null
                ? Collections.emptyList()
                : (List<Map<String, Object>>) resultado.getOrDefault("validacionesSaldo", Collections.emptyList());
        ...
        cuerpo.append("</table>");
        if (!validacionesSaldo.isEmpty()) {
            cuerpo.append("<h2>Validacion saldo de ruta vs cantidad de venta</h2>");
            cuerpo.append("<table><tr><th>Ruta</th><th>Detalle</th></tr>");
            for (Map<String, Object> validacion : validacionesSaldo) {
                cuerpo.append("<tr>");
                td(cuerpo, validacion.get("id_ruta"));
                td(cuerpo, validacion);
                cuerpo.append("</tr>");
            }
            cuerpo.append("</table>");
        }
