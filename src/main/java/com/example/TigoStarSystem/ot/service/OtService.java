...
public class OtService {
    ...
    public int registrarCargoUsuario(OtRegistrarCargoUsuarioRequest request, Integer idSucursal) {
        ...
        LocalDate fechaTrabajo = toLocalDate(findValue(ventaRow, "Fecha_Ejecucion", "fecha_ejecucion", "fecha"));
        if (fechaTrabajo == null) {
            fechaTrabajo = LocalDate.now();
        }
        ...
        for (OtCargoUsuarioItemRequest item : request.getItems()) {
            ...
            // Flujo legacy (WinForms): tambien registra movimiento tipo 41.
            if ((serie != null && !serie.isEmpty()) || (chipId != null && !chipId.isEmpty())) {
    ...
    /**
     * Registra cargo usuario y marca OT como realizada en una sola transaccion.
     */
    @Transactional
    public Map<String, Object> registrarRealizadaConCargo(
            OtRegistrarRealizadaConCargoRequest request,
            Integer idSucursal) {
        if (request == null) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "VALIDATION_ERROR", "El cuerpo de la solicitud es requerido.");
        }

        OtRegistrarCargoUsuarioRequest cargoRequest = new OtRegistrarCargoUsuarioRequest();
        cargoRequest.setNumeroOrden(request.getNumeroOrden());
        cargoRequest.setItems(request.getItems());
        int guardados = registrarCargoUsuario(cargoRequest, idSucursal);

        OtRealizadaRequest realizadaRequest = new OtRealizadaRequest();
        realizadaRequest.setNumeroOrden(request.getNumeroOrden());
        realizadaRequest.setIdEstado(request.getIdEstado());
        realizadaRequest.setObservacion(request.getObservacion());
        int filasOt = registrarOtRealizada(realizadaRequest, idSucursal);

        Map<String, Object> ventaRow = otRepository.obtenerOrdenTrabajoPorNumeroUnica(request.getNumeroOrden().trim(), idSucursal);
        Long idVenta = ventaRow == null ? null : toLong(findValue(ventaRow, "Id_Venta", "idVenta", "id_venta", "id"));

        Map<String, Object> result = new LinkedHashMap<>();
        result.put("guardados", guardados);
        result.put("filasOt", filasOt);
        result.put("idVenta", idVenta);
        result.put("numeroOrden", request.getNumeroOrden());
        return result;
    }
