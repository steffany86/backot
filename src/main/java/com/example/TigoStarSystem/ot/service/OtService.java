...
import com.example.TigoStarSystem.ot.dto.OtModificarCantidadItemRequest;
import com.example.TigoStarSystem.ot.dto.OtModificarCantidadesRequest;
...
public class OtService {
    ...
    public OtRegistrarVentaResponse registrarVentaParaRegistroOtWb(
        ...
        throw new ApiException(
                ...
                "VALIDATION_ERROR",
                "Debes adjuntar un archivo para registrar la OT."
        );
    }
    ...
    public OtRegistrarVentaResponse registrarVentaParaRegistroOtWb(
        ...
        Integer idSucursalRequest = request.getIdSucursal();
        if (idSucursalSesion != null && idSucursalSesion > 0
                && idSucursalRequest != null && idSucursalRequest > 0
                && !idSucursalSesion.equals(idSucursalRequest)) {
            throw new ApiException(
                    HttpStatus.BAD_REQUEST,
                    "VALIDATION_ERROR",
                    "La sucursal enviada no coincide con la sucursal de la sesion."
            );
        }
        final Integer idSucursalFinal = idSucursalRequest != null ? idSucursalRequest : idSucursalSesion;
        validarMayorCero(idSucursalFinal, "idSucursal");

        // The branch template has its own DataSource, so its transaction must be opened explicitly.
        return ejecutarEnTransaccionSucursal(
                idSucursalFinal,
                () -> registrarVentaParaRegistroOtWbInterno(request, idSucursalFinal, pdf)
        );
    }

    private OtRegistrarVentaResponse registrarVentaParaRegistroOtWbInterno(
            OtRegistrarVentaRequest request,
            Integer idSucursalSesion,
            MultipartFile pdf) {
        ...
        Integer idSucursalFinal = idSucursalSesion;
        ...
        Integer idSucursalResolucion = idSucursalFinal;
        ...
        try {
            ...
            if (idVentaRegistro == null || idVentaRegistro <= 0) {
                throw new ApiException(
                        ...
                        "VENTA_REGISTRO_ERROR",
                        "El registro de la OT no devolvio un Id_Venta valido."
                );
            }
            ...
            if (idVentaRegistro != null && idVentaRegistro > 0 && rutaPdf != null) {
                try {
                    ...
                    if (filas <= 0) {
                        ...
                        details.put("idVenta", idVentaRegistro);
                        details.put("idSucursal", idSucursalResolucion);
                        details.put("motivo", "El UPDATE de RutaPdf no encontro la venta creada.");
                        logger.error("No se pudo actualizar RutaPdf. idVenta={} idSucursal={} rutaPdf={}",
                                idVentaRegistro, idSucursalResolucion, rutaPdf);
                        throw new ApiException(
                                HttpStatus.INTERNAL_SERVER_ERROR,
                                "PDF_REGISTRO_ERROR",
                                "No se pudo asociar el PDF a la OT recien creada.",
                                details
                        );
                    }
                } catch (QueryTimeoutException ex) {
                    logger.error("Timeout al actualizar RutaPdf. idVenta={} idSucursal={} rutaPdf={}",
                            idVentaRegistro, idSucursalResolucion, rutaPdf, ex);
                    throw new ApiException(
                            HttpStatus.GATEWAY_TIMEOUT,
                            "PDF_REGISTRO_TIMEOUT",
                            "Se excedio el tiempo de espera al asociar el PDF a la OT. Intenta nuevamente."
                    );
                } catch (DataAccessException ex) {
                    ...
                    logger.error("Error SQL al actualizar RutaPdf. idVenta={} idSucursal={} rutaPdf={}",
                            idVentaRegistro, idSucursalResolucion, rutaPdf, ex);
                    throw new ApiException(
                            HttpStatus.INTERNAL_SERVER_ERROR,
                            "PDF_REGISTRO_ERROR",
    ...
    /** Lista las OT finalizadas hoy por el grupo propietario del usuario. */
    public List<Map<String, Object>> listarFinalizadasModificablesHoy(Integer idUsuario, Integer idSucursal) {
        LocalDate hoy = LocalDate.now();
        List<Integer> vendedores = obtenerIdsVendedorFiltro(idUsuario, idSucursal, "ModificarOT", hoy);
        List<Map<String, Object>> ventas = otRepository.obtenerVentasModificablesHoy(vendedores, idSucursal, hoy);
        Map<Integer, String> bloqueoPorRuta = new HashMap<>();
        List<Map<String, Object>> out = new ArrayList<>();
        for (Map<String, Object> venta : ventas) {
            Long idVenta = toLong(findValue(venta, "idVenta", "Id_Venta"));
            Integer idRuta = toInteger(findValue(venta, "idRuta", "Id_Ruta"));
            String bloqueo = mensajeBloqueoCantidadesCacheado(bloqueoPorRuta, idRuta, hoy, idSucursal);
            Map<String, Object> row = new LinkedHashMap<>(venta);
            row.put("materiales", otRepository.obtenerMaterialesInstaladosNoSerializados(idVenta, idSucursal));
            row.put("puedeModificar", bloqueo.isEmpty());
            row.put("bloqueo", bloqueo);
            out.add(row);
        }
        return out;
    }

    /** Obtiene una sola OT finalizada hoy con sus materiales editables actuales. */
    public Map<String, Object> obtenerFinalizadaModificableHoy(Long idVenta, Integer idUsuario, Integer idSucursal) {
        if (idVenta == null || idVenta <= 0) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "VALIDATION_ERROR", "Debe enviar una OT valida.");
        }
        LocalDate hoy = LocalDate.now();
        List<Integer> vendedores = obtenerIdsVendedorFiltro(idUsuario, idSucursal, "ModificarOT", hoy);
        List<Map<String, Object>> ventas = otRepository.obtenerVentasModificablesHoy(vendedores, idSucursal, hoy);
        for (Map<String, Object> venta : ventas) {
            Long current = toLong(findValue(venta, "idVenta", "Id_Venta"));
            if (!idVenta.equals(current)) {
                continue;
            }
            Integer idRuta = toInteger(findValue(venta, "idRuta", "Id_Ruta"));
            String bloqueo = mensajeBloqueoCantidades(idRuta, hoy, idSucursal);
            Map<String, Object> row = new LinkedHashMap<>(venta);
            row.put("materiales", otRepository.obtenerMaterialesInstaladosNoSerializados(idVenta, idSucursal));
            row.put("puedeModificar", bloqueo.isEmpty());
            row.put("bloqueo", bloqueo);
        ...
        throw new ApiException(HttpStatus.FORBIDDEN, "OT_NO_PROPIETARIA", "La OT no pertenece al grupo propietario del usuario.");
    }

    /**
     * mensajeBloqueoCantidades dispara varias consultas/EXEC (cuadre de ruta, cierres de
     * almacen). Cuando se lista un grupo de OT del mismo idRuta, calcularlo una sola vez
     * por ruta evita repetir ese costo por cada fila (antes se ejecutaba dos veces por OT:
     * una para "bloqueo" y otra, redundante, dentro de puedeModificarCantidades).
     */
    private String mensajeBloqueoCantidadesCacheado(
            Map<Integer, String> cache,
            Integer idRuta,
            LocalDate fecha,
            Integer idSucursal
    ) {
        Integer key = idRuta == null ? -1 : idRuta;
        return cache.computeIfAbsent(key, k -> mensajeBloqueoCantidades(idRuta, fecha, idSucursal));
    }

    @Transactional
    public Map<String, Object> modificarCantidadesHoy(
            Long idVenta,
            OtModificarCantidadesRequest request,
            Integer idUsuario,
            Integer idSucursal
    ) {
        if (idVenta == null || idVenta <= 0 || request == null || request.getMateriales() == null || request.getMateriales().isEmpty()) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "VALIDATION_ERROR", "Debe enviar la OT y al menos un material.");
        }
        LocalDate hoy = LocalDate.now();
        List<Integer> vendedores = obtenerIdsVendedorFiltro(idUsuario, idSucursal, "ModificarOT", hoy);
        List<Map<String, Object>> ventas = otRepository.obtenerVentasModificablesHoy(vendedores, idSucursal, hoy);
        Map<String, Object> venta = null;
        for (Map<String, Object> candidate : ventas) {
            Long current = toLong(findValue(candidate, "idVenta", "Id_Venta"));
            if (idVenta.equals(current)) {
                venta = candidate;
                break;
            }
        ...
        String bloqueo = mensajeBloqueoCantidades(idRuta, hoy, idSucursal);
        if (!bloqueo.isEmpty()) {
            throw new ApiException(HttpStatus.CONFLICT, "OT_MODIFICACION_BLOQUEADA", bloqueo);
        }

        List<Map<String, Object>> materiales = otRepository.obtenerMaterialesInstaladosNoSerializados(idVenta, idSucursal);
        Set<Long> permitidos = new HashSet<>();
        Map<Long, Map<String, Object>> materialPorId = new HashMap<>();
        for (Map<String, Object> material : materiales) {
            Long idCodigo = toLong(findValue(material, "idCodigoVenta", "Id_CodigoVenta"));
            if (idCodigo != null) {
                permitidos.add(idCodigo);
                materialPorId.put(idCodigo, material);
            }
        }
        int actualizados = 0;
        for (OtModificarCantidadItemRequest item : request.getMateriales()) {
            if (item == null || item.getIdCodigoVenta() == null || item.getCantidad() == null
                    || item.getCantidad().compareTo(BigDecimal.ZERO) < 0
                    || !permitidos.contains(item.getIdCodigoVenta())) {
                throw new ApiException(HttpStatus.BAD_REQUEST, "MATERIAL_NO_VALIDO", "Solo se pueden modificar materiales instalados no serializados de esta OT.");
            }
            Map<String, Object> material = materialPorId.get(item.getIdCodigoVenta());
            Integer idProducto = toInteger(findValue(material, "idProducto", "Id_Producto"));
            BigDecimal disponible = otRepository.obtenerSaldoDisponibleParaModificar(
                    idVenta,
                    item.getIdCodigoVenta(),
                    idRuta,
                    idProducto,
                    hoy,
                    idSucursal
            );
            if (item.getCantidad().compareTo(disponible) > 0) {
                String nombre = String.valueOf(findValue(material, "producto", "Producto"));
                throw new ApiException(
                        HttpStatus.CONFLICT,
                        "SALDO_INSUFICIENTE",
                        "La cantidad de " + nombre + " no puede ser mayor al saldo disponible (" + disponible.stripTrailingZeros().toPlainString() + ")."
                );
            }
            actualizados += otRepository.actualizarCantidadMaterialNoSerializado(
                    item.getIdCodigoVenta(), idVenta, item.getCantidad(), idRuta, idProducto, hoy, idSucursal);
        }
        if (actualizados != request.getMateriales().size()) {
            throw new ApiException(HttpStatus.CONFLICT, "MATERIAL_NO_ACTUALIZADO", "No se pudo actualizar uno o más materiales.");
        }
    ...
    /**
     * Agrega un material nuevo (serializado o no, instalado o retirado) a una OT ya finalizada
     * hoy, reutilizando las mismas validaciones de cuadre/cierre/series/saldo del registro
     * inicial de detalle (registrarDetalleAgenda), pero sin el bloqueo "detalle ya registrado".
     */
    @Transactional
    public Map<String, Object> agregarMaterialModificarHoy(
            Long idVenta,
            OtDetalleMaterialRequest material,
            Integer idUsuario,
            Integer idSucursal
    ) {
        if (idVenta == null || idVenta <= 0 || material == null) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "VALIDATION_ERROR", "Debe enviar la OT y el material a agregar.");
        }
        LocalDate hoy = LocalDate.now();
        List<Integer> vendedores = obtenerIdsVendedorFiltro(idUsuario, idSucursal, "ModificarOT", hoy);
        List<Map<String, Object>> ventas = otRepository.obtenerVentasModificablesHoy(vendedores, idSucursal, hoy);
        ...
        List<OtDetalleMaterialRequest> materiales = Collections.singletonList(material);
        validarMaterialesDetalle(materiales, idRuta, idSucursal);
        validarSaldoDisponibleMateriales(materiales, idRuta, hoy, idSucursal);

        Integer idVendedor = toInteger(findValue(venta, "idVendedor", "Id_Vendedor"));
        Integer codigoCliente = toInteger(findValue(venta, "codigoCliente", "CodigoCliente"));
        String ordenTrabajo = asString(findValue(venta, "ordenTrabajo", "OrdenTrabajo"));
        final Integer idRutaFinal = idRuta;

        ejecutarEnTransaccionSucursal(idSucursal, () -> {
            ...
            boolean esRetiradoParaDevolucion = esMaterialRetirado(material);
            logger.info(
                    "agregarMaterialModificarHoy:chequeoDevolucion idVenta={}, idProducto={}, idTipoMaterial={}, esRetirado={}, idVendedor={}, codigoCliente={}, ordenTrabajo={}",
                    idVenta,
                    material.getIdProducto(),
                    material.getIdTipoMaterial(),
                    esRetiradoParaDevolucion,
                    idVendedor,
                    codigoCliente,
                    ordenTrabajo
            );
            if (esRetiradoParaDevolucion && idVendedor != null && codigoCliente != null && ordenTrabajo != null && !ordenTrabajo.isEmpty()) {
                Integer idDevolucion = otRepository.obtenerIdDevolucionPorVenta(idVenta, idSucursal);
                if (idDevolucion != null) {
                    logger.info("agregarMaterialModificarHoy:idDevolucion existente reutilizado={} para idVenta={}", idDevolucion, idVenta);
                } else {
                    idDevolucion = otRepository.insertarDevolucion(
                            idUsuario,
                            idRutaFinal,
                            idVendedor,
                            ordenTrabajo + " - " + codigoCliente,
                            hoy,
                            "Agregado desde Modificar OT",
                            idVenta,
                            idSucursal
                    );
                    logger.info("agregarMaterialModificarHoy:idDevolucion generado={} para idVenta={}", idDevolucion, idVenta);
                }
                if (idDevolucion != null) {
        ...
        return obtenerFinalizadaModificableHoy(idVenta, idUsuario, idSucursal);
    }

    private boolean puedeModificarCantidades(Integer idRuta, LocalDate fecha, Integer idSucursal) {
        return mensajeBloqueoCantidades(idRuta, fecha, idSucursal).isEmpty();
    }

    private String mensajeBloqueoCantidades(Integer idRuta, LocalDate fecha, Integer idSucursal) {
        if (idRuta == null || idRuta <= 0) return "La OT no tiene grupo propietario válido.";
        if (resultadoIndicaRegistro(otRepository.validarCuadreRuta(idRuta, fecha, idSucursal))) return "No se puede modificar: el grupo ya tiene cuadre.";
        if (otRepository.contarCierreAlmacenDirecto(fecha, idSucursal) > 0
                || resultadoIndicaRegistro(otRepository.existeCierreAlmacenHoy(fecha, idSucursal))) return "No se puede modificar: existe cierre de almacén.";
        if (otRepository.contarCierreAlmacenPrPdDirecto(fecha, idSucursal) > 0
                || resultadoIndicaRegistro(otRepository.existeCierreAlmacenHoyPrPd(fecha, idSucursal))) return "No se puede modificar: existe cierre PR/PD.";
        return "";
    }

    private boolean resultadoIndicaRegistro(List<Map<String, Object>> rows) {
        if (rows == null || rows.isEmpty()) return false;
        for (Map<String, Object> row : rows) {
            if (row == null) continue;
            for (Object value : row.values()) {
                if (value instanceof Boolean && (Boolean) value) return true;
                if (value instanceof Number && ((Number) value).doubleValue() != 0d) return true;
                if (value instanceof String) {
                    String normalized = ((String) value).trim().toLowerCase(Locale.ROOT);
                    if ("1".equals(normalized) || "true".equals(normalized) || "si".equals(normalized) || "yes".equals(normalized)) return true;
                }
            }
        }
        return false;
    ...
    private void validarRegistroVentaRequest(OtRegistrarVentaRequest request) {
        ...
        if (request.getTieneDetalle() == null) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "VALIDATION_ERROR", "tieneDetalle es requerido.");
        }
    }
    ...
    private Integer resolverIdVendedorRegistroVenta(
            ...
            Integer idSucursal) {
        List<Integer> idsVendedor = otRepository.obtenerIdsVendedorPorIdUsuario(idUsuario, idSucursal);
        // La cabecera de la OT ya trae el vendedor seleccionado. Usarlo cuando
        // la relación usuario-técnico no está creada en esa sucursal evita
        // perder el dato entre el front y la ejecución del SP.
        if (idVendedorRequest != null && idVendedorRequest > 0
                && idsVendedor != null && idsVendedor.contains(idVendedorRequest)) {
            return idVendedorRequest;
        }
        if (idsVendedor == null || idsVendedor.isEmpty()) {
            if (otRepository.existeVendedorActivo(idVendedorRequest, idSucursal)) {
                logger.warn("Registro OT: se usa id_vendedor enviado porque no existe enlace en tbl_usuariotecnico. " +
                        "idUsuario={}, idVendedor={}, idSucursal={}", idUsuario, idVendedorRequest, idSucursal);
                return idVendedorRequest;
            }
            Map<String, Object> details = new HashMap<>();
            details.put("idUsuario", idUsuario);
        ...
        if (idsVendedor.size() > 1) {
            if (idVendedorRequest != null && idVendedorRequest > 0
                    && idsVendedor.contains(idVendedorRequest)) {
                return idVendedorRequest;
            }
            Map<String, Object> details = new HashMap<>();
            details.put("idUsuario", idUsuario);
    ...
    private void validarSaldoDisponibleMateriales(
        ...
        for (Map<String, Object> row : saldoRows) {
            ...
            if (!nombrePorProducto.containsKey(idProducto)) {
                String nombre = asString(findValue(row, "Nombre", "nombre", "producto", "Producto"));
                if (nombre != null && !nombre.trim().isEmpty()) {
                    nombrePorProducto.put(idProducto, nombre.trim());
                }
            }
        }
        ...
        for (Map.Entry<Integer, BigDecimal> entry : requeridoPorProducto.entrySet()) {
            ...
            if (saldoFinal.compareTo(BigDecimal.ZERO) < 0) {
                deficits.add(
                        nombrePorProducto.getOrDefault(idProducto, "Producto " + idProducto) +
