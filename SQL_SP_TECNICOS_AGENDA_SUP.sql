...
BEGIN
    ...
    IF OBJECT_ID(N'dbo.tbl_Grupo', N'U') IS NOT NULL
       AND OBJECT_ID(N'dbo.tbl_GrupoSup', N'U') IS NOT NULL
       AND OBJECT_ID(N'dbo.tbl_DetalleGrupo', N'U') IS NOT NULL
       AND OBJECT_ID(N'dbo.tbl_UsuarioTecnico', N'U') IS NOT NULL
       AND OBJECT_ID(N'dbo.tbl_Vendedor', N'U') IS NOT NULL
    BEGIN
        DECLARE @TecnicosGrupo TABLE (
            idTecnico INT NOT NULL,
            id_tecnico INT NOT NULL,
            tecnico NVARCHAR(250) NULL,
            ...
            codEmpleado NVARCHAR(100) NULL
        );

        INSERT INTO @TecnicosGrupo (
            idTecnico,
            id_tecnico,
            tecnico,
            nombre,
        ...
        SELECT DISTINCT
            CAST(COALESCE(v.Id_Vendedor, ut.id_Vendedor, dg.id_usuario_tecnico) AS INT) AS idTecnico,
            CAST(COALESCE(v.Id_Vendedor, ut.id_Vendedor, dg.id_usuario_tecnico) AS INT) AS id_tecnico,
            COALESCE(
                NULLIF(LTRIM(RTRIM(v.Nombre)), ''),
                'Tecnico ' + CONVERT(NVARCHAR(20), COALESCE(v.Id_Vendedor, ut.id_Vendedor, dg.id_usuario_tecnico))
            ) AS tecnico,
            COALESCE(
                NULLIF(LTRIM(RTRIM(v.Nombre)), ''),
                'Tecnico ' + CONVERT(NVARCHAR(20), COALESCE(v.Id_Vendedor, ut.id_Vendedor, dg.id_usuario_tecnico))
            ...
            gs.id_usuario AS id_encargado,
            @SucursalNorm AS sucursal,
            g.nombre AS grupo,
        ...
        WHERE gs.id_usuario = @IdUsuarioSupervisor
          ...
          AND CAST(COALESCE(v.Id_Vendedor, ut.id_Vendedor, dg.id_usuario_tecnico) AS INT) > 0;

        IF EXISTS (SELECT 1 FROM @TecnicosGrupo)
        BEGIN
            SELECT
                idTecnico,
                id_tecnico,
