  ...
  ;WITH base AS (
    SELECT r.*,
           ROW_NUMBER() OVER (
               ...
               ORDER BY
                 CASE WHEN ISDATE(r.fecha_carga)=1 THEN CONVERT(DATETIME, r.fecha_carga, 103) ELSE CONVERT(DATETIME, '19000101', 112) END DESC,
    ...
    WHERE (@FechaInicio IS NULL OR (ISDATE(r.fecha_de_respuesta)=1 AND CONVERT(DATE, r.fecha_de_respuesta, 103) >= @FechaInicio))
      AND (@FechaFin IS NULL OR (ISDATE(r.fecha_de_respuesta)=1 AND CONVERT(DATE, r.fecha_de_respuesta, 103) <= @FechaFin))
      ...
      AND (
            ...
            OR (
                 ...
                 AND (
                      ...
                      OR n.departamento_siga_norm = 'SANTA CRUZ'
                 )
               )
            OR (
                 @IdSucursal <> 9
