...
WHERE LOWER(LTRIM(RTRIM(nombre))) = 'tsm_historico_jornadas'
ORDER BY Id;
...
BEGIN
    ...
    VALUES (
        N'tsm_historico_jornadas',
        N'Historicos jornadas',
        N'HistoricoJornadasPage',
        N'/jornadas/historico',
        85,
        1,
        0,
        @now,
...
BEGIN
    ...
    SET nombre_sidebar = N'Historicos jornadas',
        pagina_asociada = N'HistoricoJornadasPage',
        Direccion = N'/jornadas/historico',
        [orden] = 85,
...
BEGIN
    IF EXISTS (
        ...
        WHERE Id_Menu = @idMenu
          AND LOWER(LTRIM(RTRIM(Pagina_Asociada))) = LOWER(N'HistoricoJornadasPage')
    )
    BEGIN
    ...
    BEGIN
        ...
        VALUES (@idMenu, N'HistoricoJornadasPage', 0, @now, 1);
    END
END
...
BEGIN
    ...
    WHERE ISNULL(E_Eliminado, 0) = 0
      AND (
            Id_Rol IN (1, 4, 6, 9, 11)
            OR LOWER(REPLACE(REPLACE(LTRIM(RTRIM(Nombre)), ' ', ''), '_', '')) IN (
                'backoffice',
                'backofficev',
                'supervisor',
                'backup',
...
SELECT
    @idMenu AS id_menu_historico_jornadas,
    tm.nombre,
    tm.nombre_sidebar,
    tm.pagina_asociada,
    tm.Direccion
