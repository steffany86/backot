-- SP pendientes Tarija como wrappers dinamicos 20260612_114112
USE [BDSistemaAntenaPMTarija];
GO

/* DYNAMIC [sp_ObtenerListaOrdenesTrabajo] */
...
/* DYNAMIC [sp_ObtenerListaOrdenesTrabajo_OTWEB] */
ALTER proc [dbo].[sp_ObtenerListaOrdenesTrabajo_OTWEB](@Fecha_Registro datetime)
AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @__sql NVARCHAR(MAX);
...
/* DYNAMIC [sp_ObtenerListaOrdenesTrabajoRFechas] */
...
/* DYNAMIC [spx_ListarOtFinalizadas] */
...
END';
    EXEC sp_executesql @__sql, N'@Fecha DATETIME, @IdUsuario INT, @IdsVendedorCsv NVARCHAR(MAX)', @Fecha = @Fecha, @IdUsuario = @IdUsuario, @IdsVendedorCsv = @IdsVendedorCsv;
END

GO

/* DYNAMIC [spx_ObtenerAuxiliaresConformacionCuadrillaWeb] */
...
/* DYNAMIC [spx_ObtenerConformacionCuadrillaWeb] */
...
BEGIN
    ...
    ORDER BY
        ...
        b.id_ruta;
END';
    EXEC sp_executesql @__sql, N'@Fecha DATE, @Sucursal NVARCHAR(100), @Limite INT', @Fecha = @Fecha, @Sucursal = @Sucursal, @Limite = @Limite;
END

GO

/* DYNAMIC [spx_ObtenerConformacionCuadrillaWebPorId] */
...
/* DYNAMIC [spx_ObtenerTecnicosConformacionCuadrillaWeb] */
...
/* DYNAMIC [spx_RegistrarVentaParaRegistroOTwb] */
...
BEGIN
    ...
    END CATCH
END';
    EXEC sp_executesql @__sql, N'@Id_Usuario INT, @Id_Vendedor INT, @Id_Grupo INT, @Id_TipoServicio INT, @OrdenTrabajo INT, @Observacion NVARCHAR(MAX), @Total DECIMAL(18,2), @Id_UsuarioE INT, @E_Eliminado BIT, @Nombre NVARCHAR(250), @Origen NVARCHAR(100), @Id_Estado INT, @Id_Sucursal INT, @CodigoCliente INT, @TieneObservacion BIT, @Latitud DECIMAL(9,6), @Longitud DECIMAL(9,6)', @Id_Usuario = @Id_Usuario, @Id_Vendedor = @Id_Vendedor, @Id_Grupo = @Id_Grupo, @Id_TipoServicio = @Id_TipoServicio, @OrdenTrabajo = @OrdenTrabajo, @Observacion = @Observacion, @Total = @Total, @Id_UsuarioE = @Id_UsuarioE, @E_Eliminado = @E_Eliminado, @Nombre = @Nombre, @Origen = @Origen, @Id_Estado = @Id_Estado, @Id_Sucursal = @Id_Sucursal, @CodigoCliente = @CodigoCliente, @TieneObservacion = @TieneObservacion, @Latitud = @Latitud, @Longitud = @Longitud;
END

GO

/* DYNAMIC [spx_ValidarVentaYDetallewb] */
...
END';
    EXEC sp_executesql @__sql, N'@Fecha DATETIME, @NroOT INT, @NumeroCliente INT', @Fecha = @Fecha, @NroOT = @NroOT, @NumeroCliente = @NumeroCliente;
END

GO

/* DYNAMIC [TraerTodosLosProductos_SinFungibleWeb] */
ALTER proc [dbo].[TraerTodosLosProductos_SinFungibleWeb]
AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @__sql NVARCHAR(MAX);
    SET @__sql = N'select * from tbl_producto  where E_Eliminado=0 and tipomaterial=''MATERIAL''
order by nombre';
    EXEC sp_executesql @__sql;
END
